    ...
    def train(self, steps):
        ...
        for _ in range(self.train_epochs):
            ...
            for i, minibatch in enumerate(minibatches):
                ...
                ##   eq.3   ##
                ##############
                critic_values = self.critic(states_tensor).squeeze(-1)  # Predicted values #고칠것
                critic_loss = torch.mean((reward_targets_tensor - critic_values) ** 2) #고칠것
                ...
                ##   eq.7   ##
                ##############
                actor_objective = torch.min(prob_ratios_tensor * reward_gaes_tensor, clipped_ratios_tensor * reward_gaes_tensor).mean() #고칠것
