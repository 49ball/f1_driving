...
class Agent(AgentBase):
    ...
    def train(self, steps):
        ...
        for train_epoch in range(self.train_epochs):
            ...
            # ============================ implement here ============================ #
            with torch.no_grad():
                entropy_alpha = self.log_entropy_alpha.exp()  ##여기도 틀렸는지 한번 보자
            ...
            with torch.no_grad():
                ...
                next_log_probs = next_action_dists.log_prob(next_actions).sum(-1, keepdim=True) ##이거한번 보자
                ...
                target_q = rewards_tensor + (1 - fails_tensor) * self.gamma * target_v
            ...
            with torch.no_grad():
                #엔트로피 계산
                entropy = self.actor(states_tensor).entropy()
                    
            self.alpha_optim.zero_grad()
            entropy_loss = -self.log_entropy_alpha * (self.target_entropy - entropy).detach()
            ...
            ##   eq.3   ##
            ##############
            # SquashedNormal 분포 객체 생성
            action_dists = self.actor(states_tensor)
            # 행동 샘플링
            actions_sampled = action_dists.rsample()
            # 분포 객체에서 샘플된 행동에 대한 로그 확률 계산
            log_probs_sampled = action_dists.log_prob(actions_sampled).sum(-1, keepdim=True)

            self.critic.eval()
            q1, q2 = self.critic(states_tensor, actions_sampled)
            q = torch.min(q1, q2)
            self.actor_optim.zero_grad()
            actor_loss = (entropy_alpha * log_probs_sampled - q).mean()
            actor_loss.backward()
